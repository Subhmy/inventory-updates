"""
UPDATE 33/11kV SUB-STATION DASHBOARD
GID: 836026436 - Automatically adds upload date
"""
...
SHEET_GID = os.getenv('SUBSTATION_33_11KV_GID')
COLLECTION_NAME = "substation_33_11kv"
...
def fetch_data():
    ...
    log_message(f"📤 Fetching 33/11kV Sub-Station data...")
...
def transform_data(data):
    ...
    for row in data:
        doc = {
            "name": safe_str(row.get('Substation Name') or row.get('Name')),
            "region": safe_str(row.get('Region')),
            "division": safe_str(row.get('Division')),
            "capacity_mva": safe_float(row.get('Capacity MVA')),
            "ptr_count": safe_int(row.get('PTR Count')),
            "feeder_count": safe_int(row.get('Feeder Count')),
            "dtr_count": safe_int(row.get('DTR Count')),
            "voltage_level": safe_str(row.get('Voltage Level') or '33/11 KV'),
...
def main():
    print("\n" + "="*60)
    print("🏭 33/11kV SUB-STATION DASHBOARD UPDATER")
    print("="*60)
